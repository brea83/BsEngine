// Source - https://stackoverflow.com/questions/5204578/is-it-possible-to-implement-events-in-c
// Posted by ceztko, modified by community. See post 'Timeline' for change history
// Retrieved 2025-12-06, License - CC BY-SA 3.0

// slight modifications to work in this engine by B 2025-12-06
#pragma once
...
// a unity style event template
template <typename TFunc>
class SubscribableEvent;

template <class RetType, class... Args>
class SubscribableEvent<RetType(Args ...)> final
{
private:
    typedef typename std::function<RetType(Args ...)> Closure;

    struct ComparableClosure // reminder to B Closures in this case are essentially referring to a callback or lambda and its caller I think
    {
        Closure Callable;
        void* Object;
        uint8_t* Functor;
...
public:
    ...
    SubscribableEvent(const SubscribableEvent& event)
    {
    ...
    void operator =(const SubscribableEvent& event)
    {
        std::atomic_store(&m_events, std::atomic_load(&event.m_events));
    }
